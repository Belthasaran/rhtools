...
class RecordCreator {
  ...
  async decodeBlob(encryptedData, keyBase64) {
    ...
    // Note: JavaScript Fernet library returns base64-encoded data, but our blob-creator
    // passed already-base64-encoded data to Fernet, so we have double base64 encoding.
    // Decode twice: once from Fernet's base64, once from our base64.
    ...
    const decompressed2 = await new Promise((resolve, reject) => {
      lzma.decompress(decoded2, (result, error) => {
        if (error) reject(error);
        else resolve(Buffer.from(result));
      });
    });
